...
const WriteSysPush = {
  ...
  refresh() {
    ...
    const altLabel     = isUpdate ? `Push New (${count})` : `Push (${count})`;
    const primaryAction = isUpdate ? 'update' : 'new';
    const altAction     = isUpdate ? 'new'    : 'update';

    this._container.innerHTML = `
      <button type="button" class="push-btn-primary" data-action="${primaryAction}">
        <svg class="push-btn-gh" viewBox="0 0 16 16" width="14" height="14" aria-hidden="true">
        ...
        <span class="push-btn-label">${primaryLabel}</span>
      </button><button type="button" class="push-btn-caret" aria-haspopup="true" aria-expanded="false">▼</button>
