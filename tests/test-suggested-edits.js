...
(async () => {
  ...
  // Wipe any leftover suggestions from a prior failed run.
  ...
  try {
    ...
    // Esc cancels (no save). Sentence is still "selected" from the revert
    // step, so a single click opens the modal directly.
    await page.locator(`.sentence[data-sentence-id="${first.id}"]`).first().click();
    await page.waitForSelector('#suggestion-modal');
