...
// data loss (4 minutes of sketch edits 403'd away by cross-tab CSRF skew):
...
(async () => {
  ...
  check('baseline save landed', psql(`SELECT text FROM sketch WHERE sketch_id=${sketchId}`).includes('first line'), '');
  ...
  for (let i = 0; i < 16 && !healed; i++) {
    ...
    healed = psql(`SELECT text FROM sketch WHERE sketch_id=${sketchId}`).includes('csrf skew');
  }
  ...
  const draft = await page.evaluate((id) => localStorage.getItem(`ms-draft-sketch-${id}`), sketchId);
  ...
  await page.unroute('**/api/sketches/*');
  ...
  for (let i = 0; i < 12 && !pushed; i++) {
    await page.waitForTimeout(500);
    pushed = psql(`SELECT text FROM sketch WHERE sketch_id=${sketchId}`).includes('PRECIOUS unsaved words');
  }
  check('restored draft pushed to the server automatically', pushed);
  const draftAfter = await page.evaluate((id) => localStorage.getItem(`ms-draft-sketch-${id}`), sketchId);
