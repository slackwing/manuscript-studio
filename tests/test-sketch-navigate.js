...
(async () => {
  ...
  await page.waitForSelector('.sn-widget .pw-actionrow-right .sn-goto-ext');
  check('navigate-to-source ↗ present in the right action row', await page.locator('.pw-actionrow-right .sn-goto-ext').count() >= 1);
  await page.locator('.pw-actionrow-right .sn-goto-ext').first().click();
  await page.waitForTimeout(800);
