...
func TestCountWords(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "Dialogue with punctuation",
			input:    `He said, 'No.'`,
			expected: 3, // He, said, No
		},
		{
			name:     "Numbers count as words",
			input:    "There are 42 apples.",
			expected: 4, // There, are, 42, apples
		},
		{
			name:     "Hyphenated words",
			input:    "The well-known author arrived.",
			expected: 5, // The, well, known, author, arrived (hyphen splits words)
		},
		{
			name:     "Code variable",
			input:    "The variable userId is important.",
			expected: 5, // The, variable, userId, is, important
		},
		{
			name:     "Single letter",
			input:    "V",
		...
		},
		{
			...
			input:    "Room 101 is open.",
			expected: 4, // Room, 101, is, open
		},
		{
			name:     "Special characters",
			input:    "Hello! @world #test $100",
			expected: 4, // Hello, world, test, 100
		},
		{
			name:     "Contractions",
			input:    "It's can't won't.",
			expected: 6, // It, s, can, t, won, t
		},
		{
			name:     "Em-dash",
			input:    "Yes—definitely—yes.",
			expected: 3, // Yes, definitely, yes
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got != tt.expected {
				t.Errorf("Expected %d words, got %d for input: %q",
					tt.expected, got, tt.input)
			}
		})
	}
}
