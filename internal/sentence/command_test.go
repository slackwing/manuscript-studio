...
func TestParseCommand_Unknown(t *testing.T) {
	cmd, ok := ParseCommand("&marker{interesting}")
	if !ok || !cmd.Unknown {
		t.Fatalf("ParseCommand(&marker{...}) = %+v, %v; want ok with Unknown=true", cmd, ok)
	}
